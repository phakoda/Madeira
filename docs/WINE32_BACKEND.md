...
The complete native AddressSanitizer suite passed on the final code revision in [run 34728436341](https://github.com/phakoda/Madeira/actions/runs/34728436341). The ARM64 iOS Simulator suite passed all four stages in [run 34728436347](https://github.com/phakoda/Madeira/actions/runs/34728436347): guest memory/thread/registry/file operations, MSI installation, relaunch of the installed EXE, and Direct3D 9. The graphics stage checked all 4096 render-target pixels, showed 17,818 green pixels in an actual Simulator screenshot, and received both Tab and Enter through the embedded input bridge. It also replaced the display host between sessions and rendered inside an inset viewport. Both device and Simulator builds passed final runtime packaging and executable linkage in that run.
...
Library tests and the complete iOS Swift type check passed in [run 34726473047](https://github.com/phakoda/Madeira/actions/runs/34726473047). No code was compiled or executed locally. The full IPA is built separately by the user's manual workflow; these targeted CI checks do not substitute for installing that IPA on a device.
